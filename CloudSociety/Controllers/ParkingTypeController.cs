using System;
using System.Web.Mvc;
using CloudSocietyEntities;
using CloudSociety.Services;


namespace CloudSociety.Controllers
{
    [Authorize(Roles = "Admin")]
    public class ParkingTypeController : Controller
    {
        private ParkingTypeService _service;
        
        const string _exceptioncontext = "Parking Type Controller";

        public ParkingTypeController()
        {
            _service = new ParkingTypeService(this.ModelState);
            
        }
        //
        // GET: /ParkingType/
       
        public ActionResult Index()
        {

            return View(_service.List());
        }

        //
        // GET: /Subscriber/Details/5
      
        public ActionResult Details(Guid id)
        {
            return View(_service.GetById(id));
        }

        //
        // GET: /ParkingType/Create

        public ActionResult Create()
        {

            return View();
        }

        //
        // POST: /ParkingType/Create

        [HttpPost]
        public ActionResult Create(ParkingType ParkingTypeToCreate)
        {
            try
            {
                //BankToCreate.BankID = System.Guid.NewGuid();

                //BankToCreate.CreatedByID = System.Guid.NewGuid();
                //BankToCreate.CreatedOn = DateTime.Now;

                if (_service.Add(ParkingTypeToCreate))
                {
                    return RedirectToAction("Index");
                }
                else
                {
                    return View();
                }
            }
            catch (Exception ex)
            {
                this.ModelState.AddModelError(_exceptioncontext + " - Get", _exceptioncontext + " " + ex.Message);
                return View();
            }
        }

        
        // GET: /ParkingType/Edit/5
     
        public ActionResult Edit(Guid id)
        {
            return View(_service.GetById(id));
        }

        //
        // POST: /ParkingType/Edit/5
    
        [HttpPost]
        public ActionResult Edit(Guid id, ParkingType ParkingTypeToUpdate)
        {
            try
            {
                // TODO: Add update logic here
                //var subscriberToUpdate = _service.GetById(id);
                //this.UpdateModel(subscriberToUpdate, collection.ToValueProvider());
                if (_service.Edit(ParkingTypeToUpdate))
                    return RedirectToAction("Index");
                else
                {

                    return View();
                }
            }
            catch (Exception ex)
            {
                this.ModelState.AddModelError(_exceptioncontext + " - Get", _exceptioncontext + " " + ex.Message);
                return View();
            }
        }

        //
        // GET: /ParkingType/Delete/5
   
        public ActionResult Delete(Guid id)
        {
            return View(_service.GetById(id));
        }

        //
        // POST: /ParkingType/Delete/5
     
        [HttpPost]
        public ActionResult Delete(Guid id, FormCollection collection)
        {
            try
            {
                if(_service.Delete(_service.GetById(id)))
                return RedirectToAction("Index");
                else
                    return View(_service.GetById(id));
            }
            catch(Exception ex)
            {
                this.ModelState.AddModelError(_exceptioncontext + " - Get", _exceptioncontext + " " + ex.Message);
                return View(_service.GetById(id));
            }
        }
    }
}

